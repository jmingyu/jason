...
class Fast_DB_NotORM /** implements PhalApi_DB */ {
     ...
     * @return array 数据库配置
     * @throws Fast_Exception_InternalServerError
     */
    protected function getDBRouter($tableName, $suffix) {
        ...
        if (empty($tableMap)) {
            throw new Fast_Exception_InternalServerError(
                'No table map config for {tableName}', array('tableName' => $tableName)
            );
        }
        ...
        if ($dbKey === NULL) {
            throw new Fast_Exception_InternalServerError(
                'No db router match for {tableName}', array('tableName' => $tableName)
            );
        }
    ...
    protected function getPdo($dbKey) {
        if (!isset($this->_pdos[$dbKey])) {
            ...
            if (empty($dbCfg)) {
                throw new Fast_Exception_InternalServerError(
                    'no such db:{db} in servers', array('db' => $dbKey));
            }
            ...
            try {
                ...
                throw new Fast_Exception_InternalServerError(
                    'can not connect to database:{db}', array('db' => $dbKey));
            }
        }
